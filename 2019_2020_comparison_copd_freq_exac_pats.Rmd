---
title: "COPD 2018-2020 Exacerbation Comparison: Frequently Exacerbating Patients"
author: "Tessa LiVoti"
date: "Oct. 20, 2020"
output:
...
report_data_query_2020 <- paste0("SELECT * FROM tessa_db.dbo.freq_exac_pat_2020 where copd_diag is not null and copd_resolved is null;")
all_pats_2020 <- sqlQuery(db, report_data_query_2020) %>% distinct()

report_data_query_2019 <- paste0("SELECT * FROM tessa_db.dbo.freq_exac_pat_2019 where copd_diag is not null and copd_resolved is null;")
all_pats_2019 <- sqlQuery(db, report_data_query_2019) %>% distinct()

report_data_query_2019 <- paste0("SELECT * FROM tessa_db.dbo.freq_exac_pat_2018 where copd_diag is not null and copd_resolved is null;")
...
d <- c(A1 = paste0("<b>Total Asthma/COPD Patients with Audit Date after Sept. 2020:       <font color = '#548235'>", format(all_pats_after_sept_2020, big.mark =
       ...
       P1 = paste("<b>Total Patients with at least 2 Exacerbations:     <font color = 'white'>", format(total_exac_march_2019, big.mark = ','),  "</font></b><br/><br/><font point-size = '9pt'>from March - Sept. 2019</font>"),
       P2 = paste("<b>Total Patients with at least 2 Exacerbations:     <font color = 'white'>", format(total_exac_march_2018, big.mark = ','),  "</font></b><br/><br/><font point-size = '9pt'>from March - Sept. 2018</font>"))
...
total_exac <- highchart() %>%
  ...
  hc_add_series(name = "March - Sept. 2020", data =  exac_all_2020, useHTML = T,
                color = '#0D5295') %>%
  hc_tooltip(pointFormat = paste('<b>{series.name}</b><br/><b>Percent of Pop</b>:{point.y}%<br/><b>Count</b>: {point.count}<br/><b>Total Populaiton</b>:15,029'), 
             useHTML = T)  %>%
