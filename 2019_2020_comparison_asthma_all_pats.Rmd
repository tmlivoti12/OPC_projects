---
title: "Asthma 2018-2020 Exacerbation Comparison"
author: "Tessa LiVoti"
date: "Oct. 20, 2020"
output:
...
report_data_query_2020 <- paste0("SELECT * FROM tessa_db.dbo.all_pats_march2020 where asthma_diag is not null and asthma_resolved is null;")
all_pats_2020 <- sqlQuery(db, report_data_query_2020) %>% distinct()

report_data_query_2019 <- paste0("SELECT * FROM tessa_db.dbo.all_pats_march2019 where asthma_diag is not null and asthma_resolved is null;")
all_pats_2019 <- sqlQuery(db, report_data_query_2019) %>% distinct()

report_data_query_2019 <- paste0("SELECT * FROM tessa_db.dbo.all_pats_march2018 where asthma_diag is not null and asthma_resolved is null;")
...
The following is a report comparing asthma patients EMR recorded exacerbations from March - September 2018, March - September 2019 and March - September 2020. The population was restricted to `r paste0("<b>", format(total_pats, big.mark = ','), "</b>")` patients from `r paste0("<b>", format(total_practices, big.mark = ','), "</b>")` practices that had an audit date after Sept. 2020.
...
total_exac <- highchart() %>%
  ...
  hc_add_series(name = "March - Sept. 2020", data =  exac_all_2020, useHTML = T,
                color = '#0D5295') %>%
  hc_tooltip(pointFormat = paste('<b>{series.name}</b><br/><b>Percent of Pop</b>:{point.y}%<br/><b>Count</b>: {point.count}<br/><b>Total Populaiton</b>: 279,541'), 
             useHTML = T)  %>%
