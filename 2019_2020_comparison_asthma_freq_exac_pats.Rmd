---
title: "Asthma 2018-2020 Exacerbation Comparison: Frequently Exacerbating Patients"
author: "Tessa LiVoti"
date: "Oct. 19, 2020"
output:
...
report_data_query_2020 <- paste0("SELECT * FROM tessa_db.dbo.freq_exac_pat_2020 where asthma_diag is not null and asthma_resolved is null;")
all_pats_2020 <- sqlQuery(db, report_data_query_2020) %>% distinct()

report_data_query_2019 <- paste0("SELECT * FROM tessa_db.dbo.freq_exac_pat_2019 where asthma_diag is not null and asthma_resolved is null;")
all_pats_2019 <- sqlQuery(db, report_data_query_2019) %>% distinct()

report_data_query_2019 <- paste0("SELECT * FROM tessa_db.dbo.freq_exac_pat_2018 where asthma_diag is not null and asthma_resolved is null;")
...
The following is a report comparing frequently exacerbating asthma patients EMR recorded exacerbations from March - September 2018, March - September 2019 and March - September 2020. The population was restricted to `r paste0("<b>", format(total_pats, big.mark = ','), "</b>")` patients from `r paste0("<b>", format(total_practices, big.mark = ','), "</b>")` practices that had an audit date after Sept. 2020.
...
total_exac_march_2020 <- nrow(all_pats_2020[(all_pats_2020$exacb_total > 1),])
total_exac_march_2019 <- nrow(all_pats_2020[(all_pats_2019$exacb_total > 1),])
total_exac_march_2018 <- nrow(all_pats_2020[(all_pats_2018$exacb_total > 1),])
...
d <- c(A1 = paste0("<b>Total Asthma/COPD Patients with Audit Date after Sept. 2020:       <font color = '#548235'>", format(all_pats_after_sept_2020, big.mark =
                                                                                                              ','),
                   "</font></b>"),
       A2 = paste("<b>Patients with Frequent Exacerbations*:     <font color = 'white'>",
                  format(freq_exac_pats, big.mark = ','),"</font></b><br/><br/><font point-size = '9pt'>*defined as patients with 2 or more exacerbations from March  2019-2020</font>"),
       A = paste("<b>Asthma Patients:      <font color = 'white'>", format(total_pats, big.mark = ','), 
                  "</font></b>"),
       P = paste("<b>Total Patients with at least 2 Exacerbations:     <font color = 'white'>", format(total_exac_march_2020, big.mark = ','),  "</font></b><br/><br/><font point-size = '9pt'>from March - Sept. 2020</font>"),
       P = paste("<b>Total Patients with at least 2 Exacerbations:     <font color = 'white'>", format(total_exac_march_2019, big.mark = ','),  "</font></b><br/><br/><font point-size = '9pt'>from March - Sept. 2019</font>"),
       P = paste("<b>Total Patients with at least 2 Exacerbations:     <font color = 'white'>", format(total_exac_march_2018, big.mark = ','),  "</font></b><br/><br/><font point-size = '9pt'>from March - Sept. 2018</font>"),
       filler = '        ')
...
graphText <- "
    digraph SampleFiltering {
        ...
        Freq [label = <@@1-2>]
        Asthma [label = <@@1-3>]
        Exacerbations_2020 [label = <@@1-4>]
        Exacerbations_2019 [label = <@@1-5>]
        Exacerbations_2018 [label = <@@1-6>]
...
graphText <- gsub('@@1-6', d[6], graphText)
...
* Asthma exacerbation 
  + Explicit asthma coded exacerbation 
...
asthma_exac_totalehr_2020 <- nrow(all_pats_2020[(all_pats_2020$exacb_asthma > 0),])
asthma_exac_codes_totalehr_2020 <- nrow(all_pats_2020[(all_pats_2020$exacb_asthma_codes > 0),])
...
asthma_exac_totalehr_2019 <- nrow(all_pats_2019[(all_pats_2019$exacb_asthma > 0),])
asthma_exac_codes_totalehr_2019 <- nrow(all_pats_2019[(all_pats_2019$exacb_asthma_codes > 0),])
...
asthma_exac_totalehr_2018 <- nrow(all_pats_2018[(all_pats_2018$exacb_asthma > 0),])
asthma_exac_codes_totalehr_2018 <- nrow(all_pats_2018[(all_pats_2018$exacb_asthma_codes > 0),])



exac_all_2020 <- data.frame(y = round(c(asthma_exac_totalehr_2020,
                                   asthma_exac_codes_totalehr_2020,
                                   ocs_exac_total_ehr_2020,
                                   ha_exac_total_ehr_2020)/total_pats*100,1),
                                  count = format(c(asthma_exac_totalehr_2020,
                                   asthma_exac_codes_totalehr_2020,
                                   ocs_exac_total_ehr_2020,
                                   ha_exac_total_ehr_2020), big.mark = ','))%>% 
  mutate(y = as.numeric(y), pointWidth = c(90, 40, 40, 40))

exac_all_2019 <- data.frame(y = round(c(asthma_exac_totalehr_2019,
                                        asthma_exac_codes_totalehr_2019,
                                        ocs_exac_total_ehr_2019,
                                        ha_exac_total_ehr_2019)/total_pats*100,1),
                            count = format(c(asthma_exac_totalehr_2019,
                                        asthma_exac_codes_totalehr_2019,
                                        ocs_exac_total_ehr_2019,
                                        ha_exac_total_ehr_2019), big.mark = ','))%>% 
  mutate(y = as.numeric(y), pointWidth = c(90, 40, 40, 40))

exac_all_2018 <- data.frame(y = round(c(asthma_exac_totalehr_2018,
                                        asthma_exac_codes_totalehr_2018,
                                        ocs_exac_total_ehr_2018,
                                        ha_exac_total_ehr_2018)/total_pats*100,1),
                            count = format(c(asthma_exac_totalehr_2018,
                                        asthma_exac_codes_totalehr_2018,
                                        ocs_exac_total_ehr_2018,
...
total_exac <- highchart() %>%
  ...
  hc_xAxis(type="category", title = list(text = 'Exacerbation Rate Comparison'), labels = list(useHTML = T), 
           categories = c('Asthma Exacerbation', 'Explicit Asthma Exacerbation Code', 
                          'Acute OCS Prescription', 'Hospital Admissions')) %>%
  ...
  hc_add_series(name = "March - Sept. 2020", data =  exac_all_2020, useHTML = T,
                color = '#0D5295') %>%
  hc_tooltip(pointFormat = paste('<b>{series.name}</b><br/><b>Percent of Pop</b>:{point.y}%<br/><b>Count</b>: {point.count}<br/><b>Total Populaiton</b>: 21,672'), 
             useHTML = T)  %>%
  hc_yAxis(labels = list(format = "{value}%"), tickInterval = 5, max = 100) %>%
  hc_size(height = 400)
...
This section breaks down the total number of asthma exacerbations by frequency. 

```{r asthma_exac}
asthma_exac_0_2020 <- nrow(all_pats_2020[(all_pats_2020$exacb_asthma == 0),])
asthma_exac_1_2020 <- nrow(all_pats_2020[(all_pats_2020$exacb_asthma == 1),])
asthma_exac_2_2020 <- nrow(all_pats_2020[(all_pats_2020$exacb_asthma == 2),])
asthma_exac_3p_2020 <- nrow(all_pats_2020[(all_pats_2020$exacb_asthma > 2),])

emr_asthma_2020 <- data.frame(y = round(c(asthma_exac_0_2020, 
                                                     asthma_exac_1_2020, 
                                                     asthma_exac_2_2020, 
                                                     asthma_exac_3p_2020)/total_pats*100,1),
                                     count = format(c(asthma_exac_0_2020, 
                                                     asthma_exac_1_2020, 
                                                     asthma_exac_2_2020, 
                                                     asthma_exac_3p_2020), big.mark = ','))

asthma_exac_0_2019 <- nrow(all_pats_2019[(all_pats_2019$exacb_asthma == 0),])
asthma_exac_1_2019 <- nrow(all_pats_2019[(all_pats_2019$exacb_asthma == 1),])
asthma_exac_2_2019 <- nrow(all_pats_2019[(all_pats_2019$exacb_asthma == 2),])
asthma_exac_3p_2019 <- nrow(all_pats_2019[(all_pats_2019$exacb_asthma > 2),])

emr_asthma_2019 <- data.frame(y = round(c(asthma_exac_0_2019, 
                                          asthma_exac_1_2019, 
                                          asthma_exac_2_2019, 
                                          asthma_exac_3p_2019)/total_pats*100,1),
                              count = format(c(asthma_exac_0_2019, 
                                               asthma_exac_1_2019, 
                                               asthma_exac_2_2019, 
                                               asthma_exac_3p_2019), big.mark = ','))

asthma_exac_0_2018 <- nrow(all_pats_2018[(all_pats_2018$exacb_asthma == 0),])
asthma_exac_1_2018 <- nrow(all_pats_2018[(all_pats_2018$exacb_asthma == 1),])
asthma_exac_2_2018 <- nrow(all_pats_2018[(all_pats_2018$exacb_asthma == 2),])
asthma_exac_3p_2018 <- nrow(all_pats_2018[(all_pats_2018$exacb_asthma > 2),])

emr_asthma_2018 <- data.frame(y = round(c(asthma_exac_0_2018, 
                                          asthma_exac_1_2018, 
                                          asthma_exac_2_2018, 
                                          asthma_exac_3p_2018)/total_pats*100,1),
                              count = format(c(asthma_exac_0_2018, 
                                          asthma_exac_1_2018, 
                                          asthma_exac_2_2018, 
                                          asthma_exac_3p_2018), big.mark = ','))

exac0_asthma <- data.frame(y = c(asthma_exac_0_2020, asthma_exac_0_2019, asthma_exac_0_2018), 
                      category = c('March - Sept. 2020', 'March - Sept. 2019', 'March - Sept. 2018'))
exac1_asthma <- data.frame(y = c(asthma_exac_1_2020, asthma_exac_1_2019, asthma_exac_1_2018), 
                      category = c('March - Sept. 2020', 'March - Sept. 2019', 'March - Sept. 2018'))
exac2_asthma <- data.frame(y = c(asthma_exac_2_2020, asthma_exac_2_2019, asthma_exac_2_2018), 
                      category = c('March - Sept. 2020', 'March - Sept. 2019', 'March - Sept. 2018'))
exac3p_asthma <- data.frame(y = c(asthma_exac_3p_2020, asthma_exac_3p_2019, asthma_exac_3p_2018), 
                      category = c('March - Sept. 2020', 'March - Sept. 2019', 'March - Sept. 2018'))


library(highcharter)
asthma_exac <- highchart() %>%
  hc_chart(type = "bar") %>%
  ...
  hc_add_series(name = "3+ Exacerbations", data = exac3p_asthma, color = '#E26F2D')%>%
   hc_add_series(name = "2 Exacerbations", data = exac2_asthma, color = '#FFA977')%>%
  hc_add_series(name = "1 Exacerbation", data = exac1_asthma, color = '#9EB9D4') %>%
  hc_add_series(name = "0 Exacerbations", data = exac0_asthma, color = '#0D5295') %>%
...
## Explicit Code for Asthma Exacerbation 
...
```{r asthma_codes_exac}
asthma_codes_exac_0_2020 <- nrow(all_pats_2020[(all_pats_2020$exacb_asthma_codes == 0),])
asthma_codes_exac_1_2020 <- nrow(all_pats_2020[(all_pats_2020$exacb_asthma_codes == 1),])
asthma_codes_exac_2_2020 <- nrow(all_pats_2020[(all_pats_2020$exacb_asthma_codes == 2),])
asthma_codes_exac_3p_2020 <- nrow(all_pats_2020[(all_pats_2020$exacb_asthma_codes > 2),])

emr_asthma_codes_2020 <- data.frame(y = round(c(asthma_codes_exac_0_2020, 
                                                    asthma_codes_exac_1_2020, 
                                                     asthma_codes_exac_2_2020, 
                                                     asthma_codes_exac_3p_2020)/total_pats*100,1),
                                     count = format(c(asthma_codes_exac_0_2020, 
                                                    asthma_codes_exac_1_2020, 
                                                     asthma_codes_exac_2_2020, 
                                                     asthma_codes_exac_3p_2020), big.mark = ','))

asthma_codes_exac_0_2019 <- nrow(all_pats_2019[(all_pats_2019$exacb_asthma_codes == 0),])
asthma_codes_exac_1_2019 <- nrow(all_pats_2019[(all_pats_2019$exacb_asthma_codes == 1),])
asthma_codes_exac_2_2019 <- nrow(all_pats_2019[(all_pats_2019$exacb_asthma_codes == 2),])
asthma_codes_exac_3p_2019 <- nrow(all_pats_2019[(all_pats_2019$exacb_asthma_codes > 2),])

emr_asthma_codes_2019 <- data.frame(y = round(c(asthma_codes_exac_0_2019, 
                                          asthma_codes_exac_1_2019, 
                                          asthma_codes_exac_2_2019, 
                                          asthma_codes_exac_3p_2019)/total_pats*100,1),
                              count = format(c(asthma_codes_exac_0_2019, 
                                          asthma_codes_exac_1_2019, 
                                          asthma_codes_exac_2_2019, 
                                          asthma_codes_exac_3p_2019), big.mark = ','))

asthma_codes_exac_0_2018 <- nrow(all_pats_2018[(all_pats_2018$exacb_asthma_codes == 0),])
asthma_codes_exac_1_2018 <- nrow(all_pats_2018[(all_pats_2018$exacb_asthma_codes == 1),])
asthma_codes_exac_2_2018 <- nrow(all_pats_2018[(all_pats_2018$exacb_asthma_codes == 2),])
asthma_codes_exac_3p_2018 <- nrow(all_pats_2018[(all_pats_2018$exacb_asthma_codes > 2),])

exac0_asthma_codes <- data.frame(y = c(asthma_codes_exac_0_2020, asthma_codes_exac_0_2019, asthma_codes_exac_0_2018), 
                      category = c('March - Sept. 2020', 'March - Sept. 2019', 'March - Sept. 2018'))
exac1_asthma_codes <- data.frame(y = c(asthma_codes_exac_1_2020, asthma_codes_exac_1_2019, asthma_codes_exac_1_2018), 
                      category = c('March - Sept. 2020', 'March - Sept. 2019', 'March - Sept. 2018'))
exac2_asthma_codes <- data.frame(y = c(asthma_codes_exac_2_2020, asthma_codes_exac_2_2019, asthma_codes_exac_2_2018), 
                      category = c('March - Sept. 2020', 'March - Sept. 2019', 'March - Sept. 2018'))
exac3p_asthma_codes <- data.frame(y = c(asthma_codes_exac_3p_2020, asthma_codes_exac_3p_2019, asthma_codes_exac_3p_2018), 
                      category = c('March - Sept. 2020', 'March - Sept. 2019', 'March - Sept. 2018'))


library(highcharter)
asthma_codes_exac <- highchart() %>%
  hc_chart(type = "bar") %>%
  hc_xAxis(type = "category", categories = c('March - Sept. 2020', 'March - Sept. 2019', 'March - Sept. 2018')) %>%
  hc_plotOptions(series = list(stacking = "percent", pointPadding = .1,
                 dataLabels = list(enabled = T, formatter = JS(paste0("function() {
                           if (this.y != 0) {
                            ...
                            return null;}}")),
                      style = list(textOutline = F)))) %>% 
  hc_add_series(name = "3+ Exacerbations", data = exac3p_asthma_codes, color = '#E26F2D')%>%
   hc_add_series(name = "2 Exacerbations", data = exac2_asthma_codes, color = '#FFA977')%>%
  hc_add_series(name = "1 Exacerbation", data = exac1_asthma_codes, color = '#9EB9D4') %>%
  hc_add_series(name = "0 Exacerbations", data = exac0_asthma_codes, color = '#0D5295') %>%
    hc_tooltip(pointFormat = '<b>{series.name}</b><br/><b>Count</b>: {point.y:,.0f}<br/><small>Total Patients: {point.total:,.0f}</small>',
               useHTML = T, headerFormat = '') %>%
    hc_yAxis(labels = list(format = "{value}%"), tickInterval = 10, minorTicks = T, minorTickInterval = 5) %>%
    hc_legend(reversed = T)%>%
